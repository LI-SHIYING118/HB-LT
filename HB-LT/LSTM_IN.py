...
class HaplotypeLSTM:
    def __init__(self, phenotype_path, haplotype_dir='.', haplotype_prefix='output_',
        ...
        """
        Modular haplotype block analyzer with LSTM processing

        Args:
            phenotype_path (str): Path to phenotype CSV file
            haplotype_dir (str): Directory containing haplotype files
    ...
    def load_haplotypes(self):
        """Load and validate haplotype blocks"""
    ...
    def create_model(self, input_length):
        """Create LSTM model for a specific block length"""
        model = Sequential([
    ...
    def evaluate_block(self, block_idx):
        """Process a single haplotype block"""
    ...
    def plot_mse(self, results):
        """Generate a line plot for mean_mse and std_mse"""
        ...
        plt.xticks(results['block'].astype(int))  # Ensure block numbers are integers
    ...
    def interactive_analysis(self):
        ...
        self.plot_mse(results)

        # Get threshold from user
        while True:
            try:
                threshold = float(input("\nEnter MSE threshold for filtering blocks: "))
                valid_min = results['mean_mse'].min()
                valid_max = results['mean_mse'].max()

                if not (valid_min <= threshold <= valid_max):
                    print(f"Threshold should be between {valid_min:.2f} and {valid_max:.2f}")
                    continue
        ...
        # Filter and save
        self.filter_and_save_blocks(results, threshold)
        return results
...
if __name__ == "__main__":
    ...
    analyzer = HaplotypeLSTM(
        phenotype_path='phenotype_fake.txt',
...
if __name__ == "__main__":
    ...
    # Generate MSE plot
    analyzer.plot_mse(results)

    # Filter blocks and save updated files
    threshold = 1.5  # Set your desired MSE threshold
    analyzer.filter_and_save_blocks(results, threshold)